#!/usr/bin/env python3
"""
Main entry point for scGPT Docker container.
...
def main():
    """Main entry point for scGPT container."""
    ...
    try:
        wrapper = SCGPTWrapper(config)
        log.info(f"Initialized scGPT wrapper for mode: {mode}")
    except Exception as e:
        log.error(f"Failed to initialize scGPT wrapper: {e}")
        sys.exit(1)
    
    # Execute the requested operation
    try:
        if mode == 'train':
            log.info("Starting scGPT training...")
            wrapper.train()
            log.info("Training completed successfully")
        elif mode == 'predict':
            log.info("Starting scGPT prediction...")
            wrapper.predict()
