...
data_url = 'https://zenodo.org/records/13350497/files/WesselsSatija2023.h5ad?download=1'
data_cache_dir = 'data/wessels23'
DATASET_NAME = 'wessels23'
CELL_TYPE = 'monocytes'
COMBO_SEPARATOR = '_'
...
# Add donor_id column (required by pipeline but constant for wessels23)
...
# ============================================================================
# STEP 4: Calculate DEGs for Single Perturbations Only
# ============================================================================
print("\n" + "="*50)
...
# ============================================================================
# STEP 5: Add Baselines for Combo Prediction
# ============================================================================
print("\n" + "="*50)
...
def add_baselines(adata, split_col='split'):
    """Add baselines for combo prediction task
    ...
    # 1. Dataset mean baseline
    print("Computing dataset mean baseline...")
    train_cells = adata[adata.obs[split_col] == 'train']
    train_non_ctrl = train_cells[~train_cells.obs['condition'].isin(control_conditions)]
    
    # Get unique donor_id + condition combinations in training set
    unique_donor_condition_combos = train_non_ctrl.obs[['donor_id', 'condition']].drop_duplicates()
    
    # For each (donor_id, condition) combo, compute mean expression
    ...
    # Average across all conditions to get dataset mean
...
ctrl_cells = adata[adata.obs['condition'].isin(control_conditions)]
if len(ctrl_cells) > 0:
...
if 'additive_baseline' not in adata.uns:
    ...
    # Create additive baseline predictions for all combo perturbations
...
import pandas as pd
import numpy as np
gene_info = pd.read_csv("data/ref/gene_info_scgpt.csv")
adata.var['symbol_scgpt'] = adata.var.gene_name
adata.var.loc[~adata.var.symbol_scgpt.isin(gene_info.feature_name), 'symbol_scgpt'] = np.nan
