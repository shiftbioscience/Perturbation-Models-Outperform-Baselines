...
data_url = 'https://zenodo.org/record/10044268/files/ReplogleWeissman2022_K562_gwps.h5ad'
data_cache_dir = './data/replogle22k562gwps'
...
# Priority perturbations to include if available (TFs were going to analyze later with ChEA)
priority_perts = [
    'NANOG', 'AR', 'ETS1', 'ZC3H11A', 'E2F6', 'IRF8', 'YY1', 'CHD1', 'SRF', 'ZKSCAN1',
    'IRF3', 'ESR1', 'FOXP2', 'POU5F1', 'HDAC2', 'VDR', 'STAT3', 'CBX3', 'MAX', 'FOS',
    'SP2', 'RCOR1', 'PML', 'CREB1', 'SALL4', 'TRIM28', 'NFE2L2', 'ZBTB7A', 'ELF1', 'NR2C2',
    'RFX5', 'NFYB', 'EGR1', 'GABPA', 'RAD21', 'FLI1', 'SUZ12', 'FOSL2', 'REST', 'BCL3',
    'RELA', 'EZH2', 'MYC', 'ZEB1', 'CEBPB', 'NELFE', 'IRF1', 'USF1', 'FOXA2', 'SPI1',
    'HNF4A', 'PPARD', 'TP53', 'NFYA', 'PPARG', 'TAF7', 'E2F1', 'TP63', 'SIN3A', 'SP1',
    'BCLAF1', 'TCF3', 'GATA2', 'BRCA1', 'KLF4', 'SOX2', 'PBX3', 'SMC3', 'TCF7L2', 'ATF3',
    'SIX5', 'ATF2', 'ZMIZ1', 'TAF1', 'NFIC', 'MYOD1', 'ZBTB33', 'SMAD4', 'FOXA1', 'CTCF',
    'FOXM1', 'RUNX1', 'STAT5A', 'CEBPD', 'USF2', 'ERG', 'GATA1', 'ZNF384', 'NRF1',
    'BHLHE40', 'KAT2A', 'UBTF', 'E2F4'
]

# Get all the perts that are not control
all_perts = adata.obs['condition'].unique()
all_perts = [pert for pert in all_perts if pert != 'control']
# Check which priority perts are available in the dataset
priority_perts_found = [pert for pert in priority_perts if pert in all_perts]
print(f"Found {len(priority_perts_found)} priority perturbations in dataset")
# Get remaining perts (not in priority list)
remaining_perts = [pert for pert in all_perts if pert not in priority_perts_found]

# Calculate how many additional perts to randomly sample
n_additional = MAX_PERTS - len(priority_perts_found)
print(f"Randomly sampling {n_additional} additional perturbations to reach {MAX_PERTS} total")

# Randomly sample additional perturbations
if n_additional > 0 and len(remaining_perts) > 0:
    n_to_sample = min(n_additional, len(remaining_perts))
    random_perts = np.random.choice(remaining_perts, size=n_to_sample, replace=False).tolist()
else:
    random_perts = []

# Combine priority + random + control
perts = priority_perts_found + random_perts + ['control']

# Select only the perts we want
print(f"Selecting {len(perts)} perturbations ({len(priority_perts_found)} priority + {len(random_perts)} random + 1 control)")
print(f"Before perturbation filtering: {adata.shape[0]} cells, {adata.shape[1]} genes")
adata = adata[adata.obs['condition'].isin(perts)]

print(f"After perturbation filtering: {adata.shape[0]} cells, {adata.shape[1]} genes")


# Add donor_id column (required by pipeline but constant for replogle22k562gwps)
